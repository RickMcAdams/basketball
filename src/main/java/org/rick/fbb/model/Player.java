...
/**
  ...
  public int getNumACCGames() {
	...
	for (int i = 0; i < games.size(); i++) {
	  if ((games.get(i).getMinutes() > 0 || games.get(i).getPoints() > 0 || games.get(i).getRebounds() > 0
			  || games.get(i).getAssists() > 0 || games.get(i).getSteals() > 0 || games.get(i).getBlocks() > 0 )&& this.isACCGame(games.get(i).getOpponent())) {
		numGames++;
	  }
	}
	return numGames;
  ...
  public String printAverages(String source) {
	return "<tr><td>" + this.getTeam() + "</td>"
			+ "<td>" + this.getId() + "</td>"
			+ "<td>" + this.getNameLink(source) + "</td><td>"
			+ round(this.avgMinutes()) + "</td><td>"
			+ round(this.avgPoints()) + "</td><td>"
			+ round(this.avgRebounds()) + "</td><td>"
			+ round(this.avgAssists()) + "</td><td>"
			+ round(this.avgSteals()) + "</td><td>"
			+ round(this.avgBlocks()) + "</td><td>"
			+ round(this.avgThrees()) + "</td></tr>";
  }
  ...
  public String printAveragesAndEfficiency(String source) {
	return "<tr><td>" + this.getTeam() + "</td>"
			...
			+ round(this.avgSteals()) + "</td><td>"
			+ round(this.avgBlocks()) + "</td><td>"
			+ round(this.avgThrees()) + "</td><td>"
			+ this.getEfficiency() + "</td></tr>";
  }

  public String printNCAAAverages(String source) {
	return "<tr><td>" + this.getRegion() + "</td><td>"
                + this.getSeed() + "</td><td>"
                        + this.getTeam() + "</td><td>"
			+ this.getId() + "</td><td>"
			+ this.getNameLink(source) + "</td><td>"
			+ round(this.avgMinutes()) + "</td><td>"
			+ round(this.avgPoints()) + "</td><td>"
			+ round(this.avgRebounds()) + "</td><td>"
  ...
  public String printNCAANames(String source) {
	return "<tr><td>" + this.getId() + "</td><td>"
			+ this.getName() + " - " + this.getTeam() + "</td></tr>";
  }
  ...
  public String printACCAverages(String source) {
	return "<tr><td>" + this.getTeam() + "</td>"
			+ "<td>" + this.getId() + "</td>"
			+ "<td>" + this.getNameLink(source) + "</td><td>"
			+ round(this.avgACCMinutes()) + "</td><td>"
			+ round(this.avgACCPoints()) + "</td><td>"
			+ round(this.avgACCRebounds()) + "</td><td>"
			+ round(this.avgACCAssists()) + "</td><td>"
			+ round(this.avgACCSteals()) + "</td><td>"
			+ round(this.avgACCBlocks()) + "</td><td>"
			+ round(this.avgACCThrees()) + "</td></tr>";
  }

  public String printACCAveragesAndEfficiency(String source) {
	return "<tr><td>" + this.getOwner() + "</td>"
			+ "<td>" + this.getTeam() + "</td>"
			+ "<td>" + this.getId() + "</td>"
			+ "<td>" + this.getNameLink(source) + "</td><td>"
			+ round(this.avgACCMinutes()) + "</td><td>"
...
//	  System.out.println("------------" + begin.getTimeInMillis());
//	  System.out.println(date.getTimeInMillis());
//	  System.out.println(end.getTimeInMillis() + "------------");
	  if ((date.after(begin) || date.equals(begin)) && (date.before(end) || date.equals(end))) { //Within the range
		//if (date.before(end1)) {
		game = games.get(i);
  ...
  public List<Game> getWeekGames(String beginDate, String endDate, boolean acc) {
	...
	end1.roll(Calendar.DATE, false); // Roll it back 1 day to get only Saturday and Sunday games
  ...
  public List<Game> getACCGames() {
	...
	for (int i = 0; i < games.size(); i++) {
	  if (isACCGame(games.get(i).getOpponent()) && (games.get(i).getMinutes() > 0 || games.get(i).getPoints() > 0 || games.get(i).getRebounds() > 0
			  || games.get(i).getAssists() > 0 || games.get(i).getSteals() > 0 || games.get(i).getBlocks() > 0)) {
  ...
  public boolean isACCGame(String opp) {
	opp = opp.toUpperCase();
	if (opp.compareTo("BC") == 0
			|| opp.compareTo("CLEMSON") == 0
			|| opp.compareTo("DUKE") == 0
			|| opp.compareTo("FLORIDA ST") == 0
			|| opp.compareTo("FSU") == 0
			|| opp.compareTo("GA TECH") == 0
			|| opp.compareTo("LOUISVILLE") == 0
			|| opp.compareTo("MIAMI") == 0
			|| opp.compareTo("UNC") == 0
			|| opp.compareTo("NC STATE") == 0
			|| opp.compareTo("VIRGINIA") == 0
					|| opp.compareTo("UVA") == 0
			|| opp.compareTo("VA TECH") == 0
			|| opp.compareTo("W FOREST") == 0
			|| opp.compareTo("BOSTON COLLEGE") == 0
			|| opp.compareTo("FLORIDA STATE") == 0
			|| opp.compareTo("GEORGIA TECH") == 0			
			|| opp.compareTo("MIAMI (FL)") == 0
			|| opp.compareTo("NORTH CAROLINA") == 0
			|| opp.compareTo("NORTH CAROLINA STATE") == 0
			|| opp.compareTo("VIRGINIA TECH") == 0
			|| opp.compareTo("WAKE FOREST") == 0
		|| opp.compareTo("NOTRE DAME") == 0
		|| opp.compareTo("PITTSBURGH") == 0
				|| opp.compareTo("PITT") == 0
  ...
  public void calculateEfficiency(float points, float rebounds, float assists,
		  float steals, float blocks, float threes) {
  ...
  public void calculateAccEfficiency(float points, float rebounds, float assists,
		  float steals, float blocks, float threes) {

	this.setAccEfficiency((this.avgACCPoints() / points) + (this.avgACCRebounds() / rebounds) + (this.avgACCAssists() / assists)
			+ (this.avgACCSteals() / steals) + (this.avgACCBlocks() / blocks) + (this.avgACCThrees() / threes));
  ...
  public void setOrder(int order) {
    ...
    /**
     * @param accEfficiency the accEfficiency to set
     */
